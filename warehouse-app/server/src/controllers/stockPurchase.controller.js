const s = require('../services/stockPurchase.service')
...
exports.create = async (req, res) => {
  try {
    ...
    if (!['Purchase', 'Purchase Return'].includes(reason)) fe.push({ field: 'reason', reason: 'must be Purchase or Purchase Return' })
    ...
    line_items?.forEach((item, i) => {
      ...
      if (reason === 'Purchase' && !(item.quantity_in > 0)) fe.push({ field: `line_items[${i}].quantity_in`, reason: 'must be > 0 for Purchase' })
      if (reason === 'Purchase Return' && !(item.quantity_out > 0)) fe.push({ field: `line_items[${i}].quantity_out`, reason: 'must be > 0 for Purchase Return' })
